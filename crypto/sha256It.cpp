#include <cstring> // For memory handling functions
#include <fstream> // For file handling
#include <iostream> // For input/output operations
#include "hash_functions.h" // Include the header file for hash functions

using namespace std; // Use the standard namespace for convenience

// Define the hash keys used in the hash function
...
// Compression function that processes the message block by block
void hash_functions::compress(const unsigned char *message, unsigned int block_nb) {
    register_32 w[64]; // Message schedule array
    register_32 buffer[8]; // Working variables
    register_32 t1, t2; // Temporary variables
    const unsigned char *sub_block; // Pointer to the current block

    // Process each block
    for (int m = 0; m < static_cast<int>(block_nb); ++m) {
...
// Initialize the state register with predefined values
...
// Adjust the digest based on the input text
...
int main(){
    ...
    cout << message << " : " << sha256(message) << endl;
    return 0;
}